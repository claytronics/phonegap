...
//args[0] = type
//args[1] = id
Item.find = function(response, args)
{
    // get cls from type
    try {
	var cls = Item.type2cls(args[0]);
    } catch (err) {
	console.error('Bad type: %s', args[0]);
	response.returnJSON({status: -1});
	return;
    }
    cls.find(args[1], function(result) {
	response.returnJSON({status:0, data:result.forFrontEnd()});
    });
}
