...
try {
    ...
    while((result = reg.exec(s)) !== null) {
	    var head = result[0];
	    var tag = result[1];
	    if (verbose) console.log('Found %s', tag);
...
// get copyright info
if (!obj.copyright) {
    console.log('No copyright field');
    copyright = "// Copyright (c) Seth Copen Goldstein, 2013, All Rights Reserved\n";
} else {
    copyright = obj.copyright;
}
if (!obj.schema) {
    console.error('No schema definition');
    Util.exit(103);
}
obj = obj.schema;
...
fs.writeSync(out, copyright);
fs.writeSync(out, templates.itemheader);
console.log('Writing ih: %s', templates.item2clsheader);
fs.writeSync(out, templates.item2clsheader);
for (var i=0; i<obj.length; i++) {
    fs.writeSync(out, "\tcase "+(obj[i].itemnum)+": cls="+obj[i].class+"; break; \n");
}
...
function createClass(obj)
{
    ...
    header = header.replace(/<itemtype>/g, obj.itemnum);
    // write out class def
